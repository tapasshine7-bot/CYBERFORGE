 ...
 * Termux/Kali-inspired practice terminal. Every command is interpreted against virtual lab data;
...
function runLabCommand(rawInput: string, lab: LabState, history: string[]): CommandResponse {
  ...
  switch (lower) {
    case "help": {
      ...
      return { lines: [entry("CYBERFORGE SAFE COMMAND DIRECTORY", "success"), entry("  help beginner | linux | termux | kali | advanced", "info"), entry("  missions / mission <1-4> / lab-status / mode termux|kali", "default"), entry("  File practice: pwd, ls, cd, mkdir, touch, rm, cp, mv, cat, grep, find", "default"), entry("  Shell concepts: ps, chmod, ip, env, pkg, apt, python3, awk, openssl, git", "default"), entry("  Security methodology: kali-tools, nmap, nikto, gobuster, sqlmap, tcpdump, burpsuite", "default"), entry("  Safety: every response is simulated; external targets, credential guessing, exploit execution, Wi-Fi capture, and device access are blocked.", "warning")] };
    }
    ...
    case "mission": {
      ...
      if (!selected) return { lines: [entry("Usage: mission <1-4>. Run `missions` to view objectives.", "warning")] };
...
function Learning({ completed, onToggle }: { completed: string[]; onToggle: (id: string) => void }) { return <section><div className="section-heading"><div><p className="eyebrow">CURRICULUM</p><h1>Learning paths</h1><p>Build from mobile shell basics through professional scope, evidence, hardening, and reporting practices. Every exercise remains inside an authorized learning boundary.</p></div></div><div className="learning-grid">{lessons.map((lesson, index) => <article className={`learning-card ${lesson.tone}`} key={lesson.id}><div className="lesson-index">{String(index + 1).padStart(2, "0")}</div><span>{lesson.level}</span><h2>{lesson.title}</h2><p>{lesson.description}</p><footer><small>{lesson.duration}</small><button className={completed.includes(lesson.id) ? "done" : ""} onClick={() => onToggle(lesson.id)}>{completed.includes(lesson.id) ? "Completed ✓" : "Mark complete"}</button></footer></article>)}</div></section>; }
